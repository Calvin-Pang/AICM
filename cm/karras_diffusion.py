...
class KarrasDenoiser:
    ...
    def consistency_losses(
        ...
        if self.sam_encoder is not None:
            ...
            img_3c = hr_inte.repeat(1, 3, 1, 1) if self.input_mode == 'single' else hr_inte[:, 1, :, :].unsqueeze(1).repeat(1, 3, 1, 1)
            img_3c = (img_3c + 1) / 2
            ...
            img_3c = img_3c.to(next(self.sam_encoder.parameters()).device)
            with torch.no_grad():
                sam_feature_list = self.sam_encoder(img_3c)
            # resize back
            sam_features = []
        ...
        def denoise_fn(x, t, hr_inte, sam_features):
            return self.denoise(model, x, t, hr_inte, sam_features)[1]

        if target_model:

            @th.no_grad()
            def target_denoise_fn(x, t, hr_inte, sam_features):
                return self.denoise(target_model, x, t, hr_inte, sam_features)[1]
        ...
        if teacher_model:
            ...
            @th.no_grad()
            def teacher_denoise_fn(x, t, hr_inte, sam_features):
                return teacher_diffusion.denoise(teacher_model, x, t, hr_inte, sam_features)[1]

        @th.no_grad()
        def heun_solver(samples, t, next_t, x0, hr_inte, sam_features):
            x = samples
            if teacher_model is None:
                denoiser = x0
            else:
                denoiser = teacher_denoise_fn(x, t, hr_inte, sam_features)
            ...
            if teacher_model is None:
                ...
                denoiser = teacher_denoise_fn(samples, next_t, hr_inte, sam_features)
        ...
        @th.no_grad()
        def euler_solver(samples, t, next_t, x0, hr_inte, sam_features):
            x = samples
            if teacher_model is None:
                denoiser = x0
            else:
        ...
        th.manual_seed(seed)
        distiller = denoise_fn(x_t, t, hr_inte, sam_features)

        if teacher_model is None:
            x_t2 = euler_solver(x_t, t, t2, x_start, hr_inte, sam_features).detach()
        else:
            x_t2 = heun_solver(x_t, t, t2, x_start, hr_inte, sam_features).detach()

        th.manual_seed(seed)
        distiller_target = target_denoise_fn(x_t2, t2, hr_inte, sam_features)
    ...
    def denoise(self, model, x_t, sigmas, hr_inte, sam_features):
        ...
        model_output = model(c_in * x_t, rescaled_t, hr_inte, sam_features)
...
def karras_sample(
    ...
    def denoiser(x_t, sigma, hr_inte, sam_features):
        _, denoised = diffusion.denoise(model, x_t, sigma, hr_inte,sam_features,  **model_kwargs)
        if clip_denoised:
...
def sample_heun(
    ...
    for i in indices:
        ...
        denoised = denoiser(x, sigma_hat * s_in, hr_inte, sam_features)
...
def sample_euler(
    ...
    for i in tqdm(indices):
        sigma = sigmas[i]
        denoised = denoiser(x, sigma * s_in, hr_inte, sam_features)
...
def sample_onestep(
    ...
    s_in = x.new_ones([x.shape[0]])
    return distiller(x, sigmas[0] * s_in, hr_inte, sam_features)
