...
import numpy as np
from MedSAM.segment_anything.modeling import ImageEncoderViT
...
def create_model(
    ...
    attention_ds = []
    if attention_resolutions != None:
        for res in attention_resolutions.split(","):
            # attention_ds.append(image_size // int(res))
            attention_ds.append(256 // int(res))
    print('attention_resolutions:', attention_ds)

    # print('use MPR-Diff UNet')
    # return UNet(
    #     in_channel = 3,
    #     out_channel = 1,
    #     inner_channel = num_channels,
    #     norm_groups = 16,
    #     channel_mults = channel_mult,
    #     attn_res = tuple(attention_ds),
    ...
    return UNetModel(
        image_size=image_size,
        in_channels= 2 if input_mode == 'single' else 4,
...
def create_ema_and_scales_fn(
    ...
    def ema_and_scales_fn(step):
        ...
        # iCM:
        elif target_ema_mode == "improved" and scale_mode == "ladder":
            target_ema = 0.
            Kp = np.floor(total_steps / (np.log2(end_scales / start_scales) + 1))
            scales = np.minimum(end_scales, start_scales * 2 ** np.floor(step / Kp)).astype(np.int32) + 1
