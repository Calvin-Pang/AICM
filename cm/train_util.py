...
class CMTrainLoop(TrainLoop):
    ...
    def run_loop(self):
        print('path:', get_blob_logdir())
        saved = False
        for epoch_id in range(self.epoch):
            logger.log(f"Epoch {epoch_id + 1}...")
    ...
    def val(self):
        ...
        val_dir_now = os.path.join(val_dir, str(self.global_step))
        ...
        with torch.no_grad():
            for i, v in enumerate(self.val_data):
                ...
                sam_features = None
                if self.diffusion.sam_encoder is not None:
                    h, w = hr_inte.shape[-2], hr_inte.shape[-1]
                    img_3c = hr_inte.repeat(1, 3, 1, 1) if hr_inte.shape[1] == 1 else hr_inte[:, 1, :, :].unsqueeze(1).repeat(1, 3, 1, 1)
                    img_3c = (img_3c + 1) / 2
                    ...
                    img_3c = img_3c.to(next(self.diffusion.sam_encoder.parameters()).device)
                    with torch.no_grad():
                        sam_feature_list = self.diffusion.sam_encoder(img_3c)
                    # resize back
                    sam_features = []
                ...
                if hr_inte.shape[1] != 1:
                    hr_inte = Image.fromarray((hr_inte[:,1,:,:].squeeze(0).squeeze(0).cpu().numpy() * 255).astype(np.uint8), mode = 'L')
                else:
                ...
                combined_img.save('{}/{}_{}.png'.format(val_dir_now, self.global_step, i + 1))
        self.target_model.train()
    ...
    def forward_backward(self, batch):
        self.mp_trainer.zero_grad()
        len_overall = len(batch['lr_img'])
        for i in range(0, len_overall, self.microbatch):
            # micro = batch[i : i + self.microbatch].to(dist_util.dev())
            micro = {}
            for k in batch.keys():    
                if isinstance(batch[k], torch.Tensor):
                    micro[k] = batch[k][i : i + self.microbatch].to(dist_util.dev())
            # micro_cond = {
            #     k: v[i : i + self.microbatch].to(dist_util.dev())
            #     for k, v in cond.items()
            # }
            last_batch = (i + self.microbatch) >= len_overall
            t, weights = self.schedule_sampler.sample(len(micro['lr_img']), dist_util.dev())
