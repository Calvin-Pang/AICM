    ...
    with torch.no_grad():
        for file_i, f in enumerate(files):
            ...
            for lr in tqdm(volume_conv_pad, leave = False):
                ...
                # hr_inte = resize_bilinear(lr, (target_h, hr_w)).unsqueeze(0).float() # 1, n*scale, w
                ...
                hr_inte_prev = resize_bicubic(lr_prev, (target_h, hr_w)).unsqueeze(0).float() # 1, n*scale, w
                hr_inte = resize_bicubic(lr, (target_h, hr_w)).unsqueeze(0).float() # 1, n*scale, w
                hr_inte_next = resize_bicubic(lr_next, (target_h, hr_w)).unsqueeze(0).float() # 1, n*scale, w

                hr_inte = torch.cat([hr_inte_prev, hr_inte, hr_inte_next], dim = 1) if args.input_mode == 'stack' else hr_inte # 1, 3, n*scale, w
                ...
                lr_img = ((hr_inte[:,1,:,:] + 1) / 2).clamp(0, 1) if args.input_mode == 'stack' else ((hr_inte + 1) / 2).clamp(0, 1)
            ...
            # volume_sr_deconv_mean = volume_sr_deconv
            # volume_lr_deconv_mean = volume_lr_deconv
            ...
            # logger.info(f"Finished processing {f} in {time_end - time_start}")
